...
//Current_LSB = _current / 2^15 
//e.g. 10 / 2^15 = 305.176u
...
{
    return (float) readInaReg(INA238_CURRENT) *  (_current / 0.032768)  *0.001;
}

float INA238::getPower_mW()
{
    return (float) readInaRegpower(INA238_POWER) * (0.2 * _current / 0.032768) * 0.001;
}
...
/*Set Shunt_CAL values for current and power calculations*/
void INA238::setShunt_CAL()
{
    int16_t    conf;
    uint16_t shunt_cal = 819200000 *  (_current / pow(2,15)) * _shunt;     
    conf = readInaReg(INA238_SHUNT_CAL);
    conf &= ~((uint16_t) 0x7FFF);
    conf |= shunt_cal ;  
    writeInaReg(INA238_SHUNT_CAL, conf);
